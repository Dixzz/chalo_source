...
public class PlayCoreMissingSplitsActivity extends Activity implements DialogInterface.OnClickListener {
    public final void onClick(DialogInterface dialogInterface, int i) {
        if (i == -1) {
            ...
            try {
                startActivity(new Intent("android.intent.action.VIEW").setData(Uri.parse(hj1.b0(new StringBuilder(String.valueOf(packageName).length() + 66), "market://details?id=", packageName, "&referrer=utm_source%3Dplay.core.missingsplits"))).setPackage("com.android.vending"));
                ...
                if (Log.isLoggable("PlayCore", 6)) {
                    nv4.c(concat, "Couldn't start missing splits activity for %s", objArr);
                }
            }
        }
        finish();
