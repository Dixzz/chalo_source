...
public abstract class AbstractAppSpiCall extends AbstractSpiCall implements AppSpiCall {
    ...
    private HttpRequest applyHeadersTo(HttpRequest httpRequest, AppRequestData appRequestData) {
        return httpRequest.header(AbstractSpiCall.HEADER_ORG_ID, appRequestData.organizationId).header(AbstractSpiCall.HEADER_GOOGLE_APP_ID, appRequestData.googleAppId).header("X-CRASHLYTICS-API-CLIENT-TYPE", "android").header("X-CRASHLYTICS-API-CLIENT-VERSION", this.version);
    }

    private HttpRequest applyMultipartDataTo(HttpRequest httpRequest, AppRequestData appRequestData) {
        HttpRequest part = httpRequest.part("org_id", appRequestData.organizationId).part(APP_IDENTIFIER_PARAM, appRequestData.appId).part(APP_NAME_PARAM, appRequestData.name).part(APP_DISPLAY_VERSION_PARAM, appRequestData.displayVersion).part(APP_BUILD_VERSION_PARAM, appRequestData.buildVersion).part(APP_SOURCE_PARAM, Integer.toString(appRequestData.source)).part(APP_MIN_SDK_VERSION_PARAM, appRequestData.minSdkVersion).part(APP_BUILT_SDK_VERSION_PARAM, appRequestData.builtSdkVersion);
    ...
    public boolean invoke(AppRequestData appRequestData, boolean z) {
        if (z) {
            ...
            StringBuilder i0 = hj1.i0("Sending app info to ");
            i0.append(getUrl());
            logger.d(i0.toString());
            try {
                HttpResponse execute = applyMultipartDataTo.execute();
                int code = execute.code();
                String str = FirebasePerformance.HttpMethod.POST.equalsIgnoreCase(applyMultipartDataTo.method()) ? "Create" : "Update";
                Logger logger2 = Logger.getLogger();
                logger2.d(str + " app request ID: " + execute.header("X-REQUEST-ID"));
