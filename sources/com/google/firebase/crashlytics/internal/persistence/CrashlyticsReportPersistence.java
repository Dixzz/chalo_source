...
public class CrashlyticsReportPersistence {
    ...
    private static String generateEventFilename(int i, boolean z) {
        return hj1.T("event", String.format(Locale.US, EVENT_COUNTER_FORMAT, Integer.valueOf(i)), z ? "_" : "");
    }
    ...
    private void synthesizeReport(File file, long j) {
        ...
        if (filesInDirectory.isEmpty()) {
            Logger logger = Logger.getLogger();
            StringBuilder i0 = hj1.i0("Session ");
            i0.append(file.getName());
            i0.append(" has no events.");
        ...
        if (arrayList.isEmpty()) {
            ...
            StringBuilder i02 = hj1.i0("Could not parse event files for session ");
            i02.append(file.getName());
            logger3.d(i02.toString());
            return;
        }
        String str = null;
        ...
        if (file2.isFile()) {
            try {
                ...
                StringBuilder i03 = hj1.i0("Could not read user ID file in ");
                i03.append(file.getName());
                logger4.d(i03.toString(), e2);
            }
        }
    ...
    public void finalizeReports(String str, long j) {
        for (File file : capAndGetOpenSessions(str)) {
            Logger logger = Logger.getLogger();
            StringBuilder i0 = hj1.i0("Finalizing report for session ");
            i0.append(file.getName());
            logger.d(i0.toString());
