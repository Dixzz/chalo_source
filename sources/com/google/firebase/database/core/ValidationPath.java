...
public class ValidationPath {
    ...
    private void checkValid() throws DatabaseException {
        if (this.byteLength > 768) {
            throw new DatabaseException(hj1.X(hj1.i0("Data has a key path longer than 768 bytes ("), this.byteLength, ")."));
        } else if (this.parts.size() > 32) {
            StringBuilder i0 = hj1.i0("Path specified exceeds the maximum depth that can be written (32) or object contains a cycle ");
            i0.append(toErrorString());
    ...
    private String toErrorString() {
        ...
        return hj1.a0(hj1.i0("in path '"), joinStringList(ConnectionFactory.DEFAULT_VHOST, this.parts), "'");
    }
